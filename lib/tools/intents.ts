...
const IMAGE_RE=/\b(?:create|generate|make|draw|design|visualize|imagine|find|search|show|browse|get)\b[\s\S]{0,90}\b(?:image|images|photo|photos|picture|pictures|illustration|artwork|poster|wallpaper|logo|icon|portrait|stock photo)\b/i;
const VIDEO_RE=/\b(?:create|generate|make|produce|animate|render|find|search|show|browse|get)\b[\s\S]{0,90}\b(?:video|videos|footage|clip|clips|animation|reel|short)\b/i;
...
export function detectRequestIntent(text:string):RequestIntent{
 ...
 if(VIDEO_RE.test(normalized))return"pexels-video";
 if(IMAGE_RE.test(normalized))return"pexels-photo";
 if(/\b(?:from|on)\s+pexels\b/i.test(normalized)){
   return /\b(?:video|videos|footage|clip|clips)\b/i.test(normalized)?"pexels-video":"pexels-photo";
 }
