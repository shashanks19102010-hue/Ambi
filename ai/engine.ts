...
export async function getLocalEngine(model: string): Promise<LocalEngine> {
  const selected = MODEL_CATALOG.some((item) => item.id === model)
    ? model
    : MODEL_CATALOG[0]?.id;
...
async function createEngine(model: string): Promise<LocalEngine> {
  if (typeof window === "undefined") {
    throw new Error("Local inference is browser-only.");
  }
  ...
  const created = await webllm.CreateMLCEngine(model, {
    initProgressCallback: (report) => {
      ...
      window.dispatchEvent(
        new CustomEvent("ambi:model-progress", { detail: progress }),
      );
    },
  });
  ...
  return {
    ...
    async *chat(messages: Message[]) {
      type CompletionRequest = Parameters<
        typeof created.chat.completions.create
      >[0];
