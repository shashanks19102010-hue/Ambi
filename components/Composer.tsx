...
export default function Composer({ onSend, onStop, busy, webSearch, onToggleResearch }: { onSend: (text: string, imageDataUrl?: string) => void; onStop: () => void; busy: boolean; webSearch: boolean; onToggleResearch: () => void }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("ambi:image-start", start); window.addEventListener("ambi:image-end", end); window.addEventListener("ambi:video-start", start); window.addEventListener("ambi:video-end", end);
  ...
  function submit() {
    ...
    if (mediaMode === "image") window.dispatchEvent(new CustomEvent("ambi:generate-image", { detail: { prompt: text } }));
      ...
      if (imageDataUrl) { try { sessionStorage.setItem("ambi:vision-image", imageDataUrl); } catch { /* the request will continue without image if storage is unavailable */ } }
