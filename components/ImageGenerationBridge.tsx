  ...
  useEffect(() => {
    const generate = async (event: Event) => {
      ...
      try {
        ...
        await new Promise((resolve) => setTimeout(resolve, 450)); setPhase("creating");
        const creating: Conversation[] = next.map((conversation) => conversation.id === active.id ? { ...conversation, messages: conversation.messages.map((message) => message.id === imageMessageId ? { ...message, generation: { type: "image", phase: "creating" } } : message) } : conversation);
        ...
        const payload = await response.json().catch(() => ({})) as { url?: string; dataUrl?: string; mediaType?: string; error?: string };
        const imageUrl = payload.url || payload.dataUrl || "";
        if (!response.ok || !imageUrl) throw new Error(payload.error || "Image generation failed.");

        setPhase("finishing");
        const finished: Conversation[] = creating.map((conversation) => conversation.id === active.id ? { ...conversation, messages: conversation.messages.map((message) => message.id === imageMessageId ? { ...message, content: "Created image", status: "complete", media: { type: "image", dataUrl: imageUrl, alt: prompt }, generation: undefined } : message), updatedAt: Date.now() } : conversation);
