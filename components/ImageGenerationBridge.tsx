  ...
  useEffect(() => {
    const generate = async (event: Event) => {
      ...
      try {
        ...
        const creating = next.map((conversation) => conversation.id === active.id ? { ...conversation, messages: conversation.messages.map((message) => message.id === imageMessageId ? { ...message, generation: { type: "image", phase: "creating", model: selectedModel } } : message) } : conversation);
        ...
        setPhase("finishing");
        const finished = creating.map((conversation) => conversation.id === active.id ? { ...conversation, messages: conversation.messages.map((message) => message.id === imageMessageId ? { ...message, content: "Created image", status: "complete", media: { type: "image", dataUrl: result.src, alt: prompt }, generation: undefined } : message), updatedAt: Date.now() } : conversation);
