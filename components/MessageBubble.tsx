...
export default function MessageBubble({ message }: { message: Message }) {
  ...
  return <article className={`message ${message.role}`}>
    ...
    <div className="message-body"><div className="meta"><span>{message.role === "user" ? "You" : "Ambi"}</span>{source && <span className="source">{source}</span>}{message.status === "streaming" && !message.generation && <span className="streaming"><span/><span/><span/></span>}</div>
      ...
      {message.pexels?.length ? <div className="pexels-results"><div className="sources-title">Pexels</div><div className="pexels-grid">{message.pexels.slice(0,12).map((item) => item.type === "photo" ? <a className="pexels-card" href={item.pexelsUrl} target="_blank" rel="noreferrer" key={item.id}><img src={item.previewUrl} alt={item.title} loading="lazy"/><div><strong>{item.title}</strong><span>Photo by {item.photographer}</span></div></a> : <a className="pexels-card pexels-video-card" href={item.pexelsUrl} target="_blank" rel="noreferrer" key={item.id}>{item.mediaUrl ? <video muted playsInline preload="metadata" src={item.mediaUrl}/> : <img src={item.previewUrl} alt={item.title} loading="lazy"/>}<div><strong>{item.title}</strong><span>Video by {item.photographer}</span></div></a>)}</div><small className="pexels-attribution">Photos and videos provided by Pexels · Open a card to view the original.</small></div>}
