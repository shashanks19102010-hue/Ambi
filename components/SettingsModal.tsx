...
export default function SettingsModal({ settings, onChange, capabilities, health, onClose, onRefreshHealth }: { settings: AppSettings; onChange: (value: AppSettings) => void; capabilities: CapabilityState | null; health: HealthState; onClose: () => void; onRefreshHealth: () => Promise<void> }) {
  ...
  const imageModel = PUTER_IMAGE_MODELS.find((m) => m.id === settings.imageModel) ?? PUTER_IMAGE_MODELS[0];
  const videoModel = PUTER_VIDEO_MODELS.find((m) => m.id === settings.videoModel) ?? PUTER_VIDEO_MODELS[0];
  const tone = healthTone(health);
  ...
  return <div className="overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><section className="modal settings-modal" role="dialog" aria-modal="true" aria-label="Ambi settings">
    ...
    <div className="settings-grid">
      ...
      <div className="field"><label htmlFor="ambi-image-model">Image model · Puter</label><select id="ambi-image-model" value={settings.imageModel} onChange={(e) => set("imageModel", e.target.value)}>{PUTER_IMAGE_MODELS.map((m) => <option value={m.id} key={m.id}>{m.name} · {m.provider}</option>)}</select><small>{imageModel.note}</small></div>
      <div className="field"><label htmlFor="ambi-video-model">Video model · Puter</label><select id="ambi-video-model" value={settings.videoModel} onChange={(e) => set("videoModel", e.target.value)}>{PUTER_VIDEO_MODELS.map((m) => <option value={m.id} key={m.id}>{m.name} · {m.provider}</option>)}</select><small>{videoModel.note}</small></div>
    ...
    <div className="row"><div><strong>Image & video generation</strong><p>Powered by Puter.js directly in the browser. No Pixazo key is required.</p></div><span className="connection-pill ready">Puter.js</span></div>
    ...
    <div className="diagnostics"><strong>Diagnostics</strong><div className="diag-grid"><div><span>Chat provider</span><b>Groq</b></div><div><span>Chat model</span><b>{model.name}</b></div><div><span>Image model</span><b>{imageModel.name}</b></div><div><span>Video model</span><b>{videoModel.name}</b></div><div><span>AI status</span><b>{health.inference}</b></div><div><span>Health</span><b>{healthLabel(health)}</b></div><div><span>Network</span><b>{health.network}</b></div><div><span>Recovery</span><b>{health.recovery}</b></div><div><span>Last recovery</span><b>{health.lastRecoveryAt ? new Date(health.lastRecoveryAt).toLocaleTimeString() : "Never"}</b></div><div><span>Research</span><b>{health.webSearch}</b></div><div><span>Device</span><b>{capabilities?.tier ?? "Detecting"}</b></div></div></div>
    <div className="modal-header settings-footer"><span className="settings-note">Puter media jobs run in the browser. Provider availability and usage can still vary; Ambi retries transient failures and shows a clear recovery state.</span><button className="primary" onClick={onClose}>Done</button></div>
  </section></div>;
}
