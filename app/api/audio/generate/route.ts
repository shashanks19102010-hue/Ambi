...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(ENDPOINT, {
      method: "POST",
      headers: { "Content-Type": "application/json", "Cache-Control": "no-cache", "Ocp-Apim-Subscription-Key": apiKey },
      body: JSON.stringify({ text, exaggeration: 0.25, temperature: 0.7, cfg: 0.5 }),
      cache: "no-store",
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Audio generation request failed." }, { status: 504 });
  }
}
...
export async function GET(request: Request) {
  ...
  if (!jobId) return NextResponse.json({ error: "Missing audio job id." }, { status: 400 });
  try {
    const response = await fetch(`${STATUS}/${encodeURIComponent(jobId)}`, { headers: { "Ocp-Apim-Subscription-Key": apiKey, Accept: "application/json" }, cache: "no-store", signal: AbortSignal.timeout(20000) });
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Audio status request failed." }, { status: 504 });
  }
}
